...
signAcademicHonestyPolicy('Christopher Chu', 'University of Wisconsin-Madison');
...
grid_size = (2 * template_radius) + 1;
grid_MN = [grid_size, grid_size];     % Number of rows and cols in the grid
...
%tracking_params.rect = chooseTarget(data_params);
 tracking_params.rect = [201 80 20 60];

% Half size of the search window
tracking_params.search_radius = 28;
...
% to specify the target of interest
%tracking_params.rect = chooseTarget(data_params);
tracking_params.rect =  [320 240 20 100];
