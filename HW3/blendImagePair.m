...
%BLENDIMAGEPAIR
    ...
    for c = 1:Cs
        ...
        elseif strcmp(mode, 'blend')
            ...
            %computing weighted mask
            %negate it because pixels closer to the edge
            %get a lower weight. We would like to reverse this.
            weighted_mask_source = bwdist(~masks);
            weighted_mask_destination = bwdist(~maskd);
            
            weighted_mask_source = cat(3, weighted_mask_source ,weighted_mask_source , weighted_mask_source );
            weighted_mask_destination = cat(3, weighted_mask_destination, weighted_mask_destination, weighted_mask_destination);
            weighted_wrapper = wrapped_imgs .* weighted_mask_source + wrapped_imgd .* weighted_mask_destination;
            out_img = weighted_wrapper ./ (weighted_mask_source + weighted_mask_destination);
