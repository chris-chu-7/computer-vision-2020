...
%               get you started.
    ...
    for n = 1:num_imgs
        ...
        img_Left = varargin{n};
        img_Right = varargin{n + 1};
        img_Center = im2double(varargin{1});
                
        
        % ---------------------------------------
        % ADD YOUR CODE HERE
        % ---------------------------------------
        % Blend img_L into stitched_img, after finding the right homography
        ...
        %1. sift finder
        [xCRight, XRight] = genSIFTMatches(img_Center, img_Right);
        [xCLeft, XLeft] = genSIFTMatches(img_Center, img_Left);
        
        RANSAC_iterations = max(length(XCRight), length(XCLeft)) / 2;
        RANSAC_epsilon = 1;
        
        [left_inliers, H_LeftC] = runRANSAC(XLeft, XCLeft, ransac_n, eps);
        [right_inliers, H_RightC] = runRANSAC(XRight, XCRight, ransac_n, eps);
       
       
        %find the homography of the left and the right bounds
        %of the stitched image
        
        [Ly, Lx, ~] = size(img_Left);
        [Ry, Rx, ~] = size(img_Right);
        
        LBound = [1, 1; Lx, 1; Lx, Ly; 1, Ly];
        RBound = [1, 1; Rx, 1; Rx, Ry; 1, Ry];
        LBoundHomo = applyHomography(H_LeftC, LBound);
        RBoundHomo = applyHomography(H_RightC, RBound);
